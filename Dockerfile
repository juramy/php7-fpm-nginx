...
# Make apt-get stop complaining about missing apt-utils in subsequent calls
RUN apt-get -y update \
    && apt-get install -y \
                apt-utils locales sudo man git htop vim mc \
                software-properties-common apt-transport-https lsb-release \
                wget lynx telnet curl parallel bzip2 \
                acl gnupg \
                exim4-daemon-light \
                supervisor nginx
